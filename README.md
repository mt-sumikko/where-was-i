...
`bookmarklet.js` は、クリップボードコピーに成功した直後、`location.href = 'shortcuts://run-shortcut?name=' + encodeURIComponent(SHORTCUT_NAME) + '&x-success=' + encodeURIComponent('x-safari-' + url)` でショートカットアプリを自動起動する。ユーザー操作としては、ブックマークレットを1回タップするだけで、あとは自動的にショートカットが起動してメモに追記される想定（環境によっては初回に外部アプリを開く確認ダイアログが挟まる可能性がある）。

`x-success` パラメータには、しおり位置を示す **text fragment付きのURL**（`url` 変数。プレーンな `location.href` ではない）を渡している。これはiOSショートカットのURLスキームが対応するx-callback-url形式のパラメータで、**ショートカットの実行完了後に指定したURLへ自動で遷移して戻る**機能。これにより「ブックマークレットタップ→ショートカットアプリに切り替わったまま」ではなく「切り替わって実行完了後、自動でSafariの元のページ（かつしおりを挟んだ位置）に戻ってくる」という体験になる想定。

さらに先頭に **`x-safari-`** というプレフィックスを付けている。`x-success` にそのまま `https://...` を渡すと、端末の既定のブラウザ設定（Safari以外、例えばChromeなど）でそのURLが開かれてしまう。`x-safari-https://...` の形にすると、既定ブラウザの設定に関わらずSafariで強制的に開かせられる（iOSの仕様）。`shortcuts://` の実際の遷移挙動、`x-safari-` の効果ともにiPhone実機での確認が必要。
